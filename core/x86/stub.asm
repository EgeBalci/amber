...
start:                      ;
	...
	jz fin				    ; If VirtualAlloc fails don't bother :/	
	push eax                ; Save the new image base to stack
;#- relocate.asm ------------------------------
; ESI = &PE
; RBP = &api.asm
; EBX = image_base
; ECX = end_of_base_relocation_table
; EDI = base_relocation_delta
; STACK[0] = address_of_entry
; STACK[1] = new_image_base
; STACK[2] = reloc_rva
; STACK[3] = reloc_table_size
; 	
...
;
	...
	mov edx,[esp] 			; Copy the address of new base to ebx
	mov ecx,image_size
memcpy:	
...
CreateThread:
	pop edi                 ; Pop the new image base to edi
	pop edx					; Pop the address of entry to edx
	add edi,edx				; Add the address of entry to new image base
